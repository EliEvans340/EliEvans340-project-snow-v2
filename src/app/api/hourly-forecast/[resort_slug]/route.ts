...
import {
  fetchAllModelsHourly,
  MultiModelHourlyResponse,
} from "@/lib/weather/multi-model-hourly-client";
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Check for cached multi-model hourly data
    const [existingSnapshot] = await db
      ...
      .where(
        and(
          ...
          eq(forecastSnapshots.model, "multi-model-hourly"),
    ...
    let multiModelData: MultiModelHourlyResponse;

    if (existingSnapshot?.rawData) {
      multiModelData =
        existingSnapshot.rawData as unknown as MultiModelHourlyResponse;
    } else {
      console.log(
        `Fetching multi-model hourly forecast for resort: ${resort.slug}`
      );

      const lat = parseFloat(resort.latitude);
      const lng = parseFloat(resort.longitude);
      const tz = resort.timezone || "America/Los_Angeles";

      multiModelData = await fetchAllModelsHourly(lat, lng, tz);
